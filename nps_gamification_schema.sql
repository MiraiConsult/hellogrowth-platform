...
CREATE TABLE IF NOT EXISTS nps_games (
  ...
  name TEXT NOT NULL,
  type TEXT NOT NULL DEFAULT 'wheel', -- 'wheel' para roleta, futuramente 'scratch', 'slot'
  status TEXT NOT NULL DEFAULT 'active', -- 'active' ou 'inactive'
  google_review_url TEXT,
  prizes JSONB NOT NULL DEFAULT '[]', -- Array de prêmios: [{name, probability, color}]
  messages JSONB NOT NULL DEFAULT '{}', -- {before_game, after_game}
...
CREATE TABLE IF NOT EXISTS nps_game_participations (
  ...
  lead_id UUID REFERENCES leads(id) ON DELETE SET NULL, -- Opcional, se vier de pesquisa NPS
  client_name TEXT NOT NULL,
  client_email TEXT,
  client_phone TEXT,
  prize_won TEXT NOT NULL,
  prize_code TEXT NOT NULL UNIQUE,
  status TEXT NOT NULL DEFAULT 'pending', -- 'pending', 'sent', 'redeemed'
